...
使用 deepagents.create_deep_agent 构建，带三个专业子 Agent：
...
GLOBAL_GUARDRAILS_SECTION = dedent(
    ...
    - 无 URL → 写 `[链接不可用]`
    - **绝不**补写、猜测或构造任何 URL
    - 融资表、玩家表、关键事实表中的主结论**优先只保留带可点击 URL 的条目**；无 URL 的条目只能进入“待核实/弱信号”，不能与已验证事实混排
    ...
    **输出前自检**：如果你的引用/表格行数明显少于子 Agent 返回的数量，你正在压缩——停下来，把漏掉的补全。
    ...
    ### 5. 数据截止日期声明（必须在输出第一段）
    任何模式下，输出的**第一段**必须包含：
    > **数据截止日期**：[本次检索/验证完成日期，格式 YYYY-MM-DD，通常与生成时间同日] | **数据来源**：[使用了哪些子 Agent] | **生成时间**：见 Prompt 末尾系统时间

    注意：
    - `数据截止日期` 不是“最新事件发生日期”，也不是文章里的预测日期
    - 如果回答的是“最近融资/最新动态”，数据截止日期通常应与本次生成日期一致
    - 事件本身发生在什么时间，应在正文单独写明，不要混写到截止日期里
    """
).strip()

MODE_SELECTION_SECTION = dedent(
    """\
    ## 意图识别 → 模式选择
    ...
    **判断优先级**：模式 0 > 模式 1 > 模式 2
    - 只要出现决策词汇（值得/布局/时机/推荐/应不应该），无论问题同时包含多少信息查询特征，**优先触发模式 1**
    - 模式 2 仅在确认不含任何决策意图时才触发
    """
).strip()

MODE_DIRECT_FORWARD_SECTION = dedent(
    """\
    ### 模式 0：直接转发（单步查询）
...
MODE_DECISION_SECTION = dedent(
    """\
    ### 模式 1：决策研判模式（完整投研）
    **触发条件**：问题含决策/建议词汇
    词汇示例：值得/应该/值不值得投/怎么布局/推荐/怎么配置/长期战略/帮我全面研究

    **执行步骤**：
    1. 用 `write_todos` 拆解任务
    2. 三个子 Agent **并行**调用：paper-researcher + people-intel + news-market
    3. 汇总三路成果，**严格遵循已加载的 investment-research SKILL** 完成报告结构、数据治理和引用编排，不再自行发明章节或评分体系
    4. 输出完成后立即调用 `save_research_artifact()` 持久化

    示例问题：“帮我全面研究量子计算赛道”、“本源量子值不值得投？”、“怎么布局量子云平台领域？”
    """
).strip()

MODE_INFORMATION_SECTION = dedent(
    """\
    ### 模式 2：信息查询模式（全面摘要）
    **触发条件**：纯信息查询，无决策意图，且需要**多维度信息**（不是单步可完成的）
    词汇示例：怎么样/什么时候/有哪些公司/发展到什么阶段/现在情况怎么样
    ...
    **输出格式**（面向投资经理，全面、可靠、不丢数据）：
    ...
    3. **市场与商业化**（仅当调用了 news-market 时输出本章）
       ...
       - 对赛道/平台/生态类问题，必须同时呈现“头部玩家”和“近12个月新进入者/新成立公司/早期团队”两层信息，不能只停留在头部名单
       - 融资事件表：子 Agent 返回了 N 行就呈现 N 行，不截断
       - 每条保留原始来源 URL
       - 无可点击 URL 的关键事实，单独归入“待核实信号”，不要混入已验证主表
    4. **人才与团队**（仅当调用了 people-intel 时输出本章）
       - 直接采用 people-intel 返回的科学家档案、机构评估、人才流动记录
    4.5 **投资含义与风险**（若为赛道全景/商业化问题，必须输出）
       - 明确写出 2-4 条对投资经理有用的判断：谁值得跟踪、哪里仍待验证、哪些环节存在空白机会
       - 每条判断后跟来源编号，不允许空泛表述
...
SUBAGENT_CAPABILITIES_SECTION = dedent(
    """\
    ## 子 Agent 能力简介
...
PROMPT_SECTIONS = (
    PROMPT_HEADER,
    GLOBAL_GUARDRAILS_SECTION,
    MODE_SELECTION_SECTION,
    MODE_DIRECT_FORWARD_SECTION,
    MODE_DECISION_SECTION,
    MODE_INFORMATION_SECTION,
    SUBAGENT_CAPABILITIES_SECTION,
...
def _compose_prompt_sections() -> str:
    """Assemble the stable orchestrator rules into one static prompt body."""
...
ORCHESTRATOR_SYSTEM_PROMPT = _compose_prompt_sections()
...
def _load_skill_files() -> dict:
    ...
    这里只加载 orchestrator 自身依赖的 investment-research，避免把子 Agent 的私有规则和
    编排器级规则混在一起，减少“加一条约束就继续往大 prompt 里堆”的维护熵增。
    """
    ...
    for virtual_prefix in _SKILL_PATHS:
        ...
        for md_file in disk_dir.rglob("*.md"):
            ...
            try:
                ...
                logger.warning("Skill 文件加载失败 %s: %s", md_file, exc)
    return files
...
def get_skill_files(*, force_reload: bool = False) -> dict:
    """Return cached orchestrator skill files for API injection.

    Skill loading and cache invalidation live here so the API layer does not
    maintain a second copy of skill state.
    """
    global _skill_files_cache
    if force_reload or _skill_files_cache is None:
        _skill_files_cache = _load_skill_files()
        logger.info("Orchestrator skill cache %s，共 %d 个文件",
                    "refreshed" if force_reload else "initialized",
                    len(_skill_files_cache))
    return dict(_skill_files_cache)


def reload_skill_files() -> dict:
    """Explicitly refresh cached orchestrator skill files."""
    return get_skill_files(force_reload=True)
